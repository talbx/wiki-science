...
public class JSONRedirectsMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
        ...
        if (wikiArticle.getText().contains("may refer to:") && wikiArticle.getText().length() < 100) {
            output.collect(new Text("redirects"), new IntWritable(1));
        }
    }
}
